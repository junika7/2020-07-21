...
def to_json(l: list, file_name: str) -> None: #tsv 파일을 처리한 파일을 json 파일로 저장하는 함수, 파일명도 지정할 수 있도록 만들었음
    with open(file_name, 'w') as handle:
        json.dump(l, handle, indent=4)

def read_json(file_name: str) -> list: #다른 스크립트에서 import readtxt 후 readtxt.read_json("~~~~") 하면 사용 가능
    with open(file_name, 'r') as handle:
        l = json.load(handle)
    return l
